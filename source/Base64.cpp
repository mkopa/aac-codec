...
uint16_t* gpLookup16= 0;

void SetupLookup16()
{
   int nLenTbl= 256*256;            // yes, the table is 128Kb!
   if (NULL==gpLookup16) {
      gpLookup16= new uint16_t[nLenTbl];
   }
   uint16_t* p= gpLookup16;
   for (int j= 0; j<256; j++ ) {
      for (int k= 0; k<256; k++ ) {
         uint16_t w;
         w  = LookupDigits[k] << 8;
...
int FromBase64Fast( const uint8_t* pSrc, int nLenSrc, uint8_t* pDst, int nLenDst )
{
   ...
   uint16_t* pwSrc= (uint16_t*)pSrc;
   for( int j=0; j<nLoopMax; j++ ) {
      uint16_t s1= gpLookup16[ pwSrc[0] ];  // translate two "digits" at once
      uint16_t s2= gpLookup16[ pwSrc[1] ];  // ... and two more
      ...
      uint8_t b3= ( n32 & 0x00ff ); n32 >>= 8;  // in reverse (WORD order)
      uint8_t b2= ( n32 & 0x00ff ); n32 >>= 8;
      uint8_t b1= ( n32 & 0x00ff ); 
   ...
   uint16_t s1= gpLookup16[ pwSrc[0] ];
   uint16_t s2= gpLookup16[ pwSrc[1] ]; 
...
std::string ToBase64Fast(const uint8_t* pSrc, int nLenSrc)
{
   ...
   //       return( 0 ); // fail!
   // }
   char outBuff[nLenOut + 1];
   char *pDst = outBuff;
   uint32_t* pdwDst= (uint32_t*)pDst;
   while( nLenSrc > 2 ) {
      uint32_t n= pSrc[0];  // xxx1 // roll the data in...
      n <<= 8;           // xx1x
      n |= pSrc[1];      // xx12  
      n <<= 8;           // x12x
      n |= pSrc[2];      // x123  

      uint8_t m4= n & 0x3f;  n >>= 6;  // roll it out, 6 bits at a time
   ...
   return std::string((const char *)&outBuff[0], nLenOut);
   // return( nLenOut );
}
// int ToBase64Fast( uint8_t* pSrc, int nLenSrc, char* pDst, int nLenDst )
// {
//    int nLenOut= ((nLenSrc+2)/3)*4; // 4 out for every 3 in, rounded up
//    if ( nLenOut+1 > nLenDst ) {
//          return( 0 ); // fail!
